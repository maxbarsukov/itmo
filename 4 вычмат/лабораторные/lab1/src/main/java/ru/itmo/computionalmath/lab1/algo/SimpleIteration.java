...
public class SimpleIteration {
  ...
  public static void compute(Matrix matrix, double eps) {
    ...
    if (checkDiagonal(matrix.getMatrix(), matrix.getSize())) {
      ...
      pr.printVector(CYAN_BOLD + "Решение системы: " + RESET, rs.getResult());
      pr.printVector(CYAN_BOLD + "Вектор невязки: " + RESET, rs.getResiduals());
      return;
    }
    ...
    if (val != null) {
      ...
      pr.println(BLUE + "Матрица после перестановки строк" + RESET);
      ...
      pr.println(RED + "Отсутствие диагонального преобладания" + RESET);
    }
  }
